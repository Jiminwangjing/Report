...
namespace CKBS.Models.Services.Responsitory
{
    ...
    public class POSRepository : IPOS
    {
        ...
        public POSRepository(DataContext context, IHubContext<SignalRClient> timehubcontext, IReport report,
            ...
            IDataPropertyRepository dataProperty)
        {
            _context = context;
            _report = report;
            _accessor = accessor;
        ...
        public void IssuseCommittedMaterial(int orderid)
        {
            ...
            var OrderDetail = _context.OrderDetail.AsNoTracking().Where(w => w.OrderID == orderid).ToList();
        ...
        public async Task<ItemsReturnObj> SendOrderAsync(Order data, string print_type, List<SerialNumber> serials = null, List<BatchNo> batches = null)
        {
            ...
            if (data.OrderID <= 0)
            {
                ...
                if (print_type == "Send") SendToPrintOrder(data, "Send");
                else SendToPrintOrder(data);
            ...
            {
                if (data.Status == OrderStatus.Pay)
                {
                    data.Status = OrderStatus.Pay;
                }
                else
                {
                    data.Status = OrderStatus.Order;
                    data.CheckBill = 'N';
                }
                _context.Order.Update(data);
                _context.SaveChanges();
        ...
        public async Task<int> InsertReceiptAsync(int orderid, List<SerialNumber> serials, List<BatchNo> batches, List<MultiPaymentMeans> multiPayments, PointTemplate point=null)
        {
            ...
            var disCurrency = _context.DisplayCurrencies.Where(w => w.PriceListID == order.PriceListID && w.IsShowCurrency == true && w.IsShowCurrency == true);
            ...
            foreach (var item in order.OrderDetail)
            {
                ...
                {
                    var ksSetUpExisted = _context.KSServices.FirstOrDefault(i => i.KSServiceSetupId == receiptDetail.KSServiceSetupId && i.CusId == receipt.CustomerID);
                    if (ksSetUpExisted != null)
                    {
            ...
            if (order.Change > 0)
            {
                ...
                if (objcmp.PaymentMeanID == 0)
                {
                    ...
                    obj.Amount = (-1) * (decimal)order.Change;
                    obj.OpenAmount = (-1) * (decimal)order.Change;
                    obj.Total = (-1) * (decimal)order.Change;
                ...
                else
                {
                    foreach (var i in multiPayments)
                    {
                        ...
                        {
                            i.Amount = i.Amount - (decimal)order.Change;
                            i.OpenAmount = i.OpenAmount - (decimal)order.Change;
                            i.Total = i.Total - (decimal)order.Change;
                            break;
                        }
                    }
                }
            ...
            IssuseInStock(receiptid, order.Status, serials, batches, multiPayments);
            return receiptid;
        }
