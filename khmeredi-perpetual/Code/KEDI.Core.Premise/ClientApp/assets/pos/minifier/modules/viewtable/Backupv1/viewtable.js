...
; (function (global, factory) {
    ...
    this.createColumn = function (json, fieldName, value) {
        let $col = $(clone("td"));
        ...
        if (__setting.columns.length > 0) {          
            for (let k = 0; k < __setting.columns.length; k++){            
                ...
                if (_name === _fieldName || _name.split(".")[0] === _fieldName) {
                    ...
                    if (_name === _fieldName) {
                        insertTemplate(json, __setting.columns[k], fieldName, value, $col);
                    }
    ...
    function defineNestedField(json, field, fieldName, $col) {
        if (isValidJson(json[fieldName]) && field.name.indexOf(".") > 0) {
            ...
            insertTemplate(json, field, fieldName, json[fieldName][field.name.split(".")[1]], $col);
        }
    }
    ...
    this.createRow = function (item, index) {
        let $row = $(clone("tr"));
        if (isValidJson(item)) {
            defineReadonly(item, __setting.keyField);
    ...
    this.addRowAfter = function (json, relatedKey, callback) {
        ...
        if (existedItem) {
            ...
            if (typeof callback === "function") {
                callback.call(__this, fallbackRow(json, $newRow[0]));
            } 
            $(row).after($newRow);
    ...
    function insertTemplate(data, field, fieldName, value, col) {
        if (isValidJson(field)) {
