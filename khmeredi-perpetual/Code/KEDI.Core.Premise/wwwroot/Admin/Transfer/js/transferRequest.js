...
$(document).ready(function () {
 ...
 $("#next_number").on("keypress", function (e) {
        ...
        if (e.which === 13) {
            $.ajax({
                url: "/Transfer/FindTransferRequest",
                data: { number: $(this).val().trim()},
                success: function (data) {
                    if(data.ID==0)
                    {
                            ...
                            new DialogBox({
                                caption: "Searching",
                                icon: "danger",
                                content: "Transfer Request Not found!",
                                close_button: "none"
                            });
...
function saveData(_this) 
{
    ...
    if (isValidArray(tb_item_manage)) {
        ...
        var data = {
            ...
            Time: time,
            SeriseID: seriesId,
            TransferRequestDetails:data_itemM_show,
        }
      
        $.ajax({
            url: "/Transfer/SaveTransferRequest",
            type: "POST",
            dataType: "JSON",
            data: {
                transfer: data,
