...
$(document).ready(function () {
    ...
    console.log("___PSJE.seriesPS",___PSJE.seriesPS)
    ...
    $("#selectall").click(function () 
    {
        ...
        if ($(this).prop("checked") == true) {
            ...
            for (let i = 0; i < txtPayment.length; i++) {
                const id = $(`.cashDis:nth(${i})`).parent().parent().data("key");
        ...
        $.get("/Outgoingpayment/GetPurchaseAP", { VendorID: vendorId }, function (resp) {
            ...
            if (countClick <= 1) {
                for (let i = 0; i < $(".totalPay").length; i++) {
                    txtPayment.push({
                        ID: $(`.cashDis:nth(${i})`).parent().parent().data("key"),
    ...
    // function Copy OutgoingOrder
    function DialogOutgoingOrder()
    {
        let dialogoutgoingOrder = new DialogBox({
            button: {
                ok: {
                    text: "Close",
                    callback: function () {
            ...
            content: {
                selector: "#container-outgingOrder"
            }
        });
        dialogoutgoingOrder.invoke(function () {
            let $outgoingOrder = ViewTable({
                keyField: "ID",
                selector: ".item-copy",
                indexed: true,
                paging: {
                ...
                actions: [
                    {
                        ...
                        on: {
                            "click": function (e) {
                                ...
                                details=[];
                               $.get("/Outgoingpayment/CopyOutgoingOrder",{id:e.data.ID},function(res){
                                    
                                    $("#baseonid").val(res.OutgoingPayOrder.ID);
                                    $("#txtref_no").val(res.OutgoingPayOrder.Ref_No);
                                    $("#txtUser").val(res.OutgoingPayOrder.UserName);
                                    $("#UserID").val(res.OutgoingPayOrder.UserID);
                                    $("#paymentMeansID").val(res.OutgoingPayOrder.PaymentMeanID);
                                    $("#paymentMeans").val(res.OutgoingPayOrder.PaymentMeanName);
                                    $("#txtPostingdate").val(res.OutgoingPayOrder.PostingDate.toString().split("T")[0]);
                                    $("#txtDocumentDate").val(res.OutgoingPayOrder.DocumentDate.toString().split("T")[0]);
                                    $("#txtremark").val(res.OutgoingPayOrder.Remark);
                                    $("#branch").val(res.OutgoingPayOrder.BranchID);

                                    let grandtotalSys=0;
                                    res.OutgoingPaymentDetails.forEach(i=>{
                                        grandtotalSys+=i.TotalPayments;
                                        i.Totals          = i.CurrencyName+" "+num.formatSpecial(i.Totals,disSetting.Amounts);
                                        i.Applied_Amounts = i.CurrencyName+" "+num.formatSpecial(i.Applied_Amounts,disSetting.Amounts);
                                        i.BalanceDues     = i.CurrencyName+" "+num.formatSpecial(i.BalanceDues,disSetting.Amounts);
                                        i.CashDiscounts   = i.CurrencyName+" "+num.formatSpecial(i.CashDiscounts,disSetting.Amounts);
                                        i.TotalDiscounts  = i.CurrencyName+" "+num.formatSpecial(i.TotalDiscounts,disSetting.Amounts);
                                        i.TotalPayments   = i.CurrencyName+" "+num.formatSpecial(i.TotalPayments,disSetting.Amounts);
                                      
                                        details.push(i);
                                    });
                                    $("#totalamount_due_asing").val(num.formatSpecial(grandtotalSys,disSetting.Amounts));
                                    $(".currency_text").text(res.OutgoingPaymentDetails[0].CurrencyName);
                                    $list_outgoing_payment_details.clearRows();
                                    $list_outgoing_payment_details.bindRows(res.OutgoingPaymentDetails);
                                    details.forEach(i=>{
                                        $list_outgoing_payment_details.disableColumns(i.OutgoingPaymentVendorID,["CheckPay"]);
                                    })

                               });
                               $("#selectall").prop("checked",true).prop("disabled",true);
                               
                                dialogoutgoingOrder.shutdown();
                            }
                        }
                    }
                ]
            ...
            $.get("/Outgoingpayment/GetOutgoingOrder",{vendorId: parseInt(vendorID)}, function (resp) {
                resp.forEach(i=> {
                    i.PostingDate= i.PostingDate.toString().split("T")[0];
                    i.TotalPayment=num.formatSpecial(i.TotalPayment,disSetting.Amounts);
                    i.TotalAmountDue=num.formatSpecial(i.TotalAmountDue,disSetting.Amounts);
                })
                $outgoingOrder.clearRows();
            
                $("#txtSearch-item-copy").keyup(function () {
                    let input = this.value.replace(/\s+/g, '');
                    let rex = new RegExp(input, "gi");
                    var filtereds = $.grep(resp, function (item, i) {
                        return item.NumberInvioce.toLowerCase().replace(/\s+/, "").match(rex) ||
                            item.VendorName.toLowerCase().replace(/\s+/, "").match(rex);
                    });
                    $outgoingOrder.bindRows(filtereds);
                })
                $outgoingOrder.bindRows(resp);
    ...
    function saveOutgoing() {
        ...
        updateMaster(master, "BaseOnID", parseInt(baseonID));
        ...
        $.ajax({
            url: "/Outgoingpayment/SaveOutgoingPayment",
            type: "Post",
            dataType: "Json",
    ...
    function findOutgoingPaymentToCancel(invoiceNumber, success) {
        ...
        if (invoiceNumber !== null) {
            $.get("/outgoingpayment/getbyinvoice", { invoiceNumber, seriesID }, success);
        }
    }
    let _seriesDID = 0;
    ...
    function cancelOutgoingInvoice(invoice, success) {
        ...
        $.post("/outgoingpaymenthistory/cancel", { invoice, seriesJE: JSON.stringify(__singleElementJE), seriesDID }, success);
    }
    //format currency
