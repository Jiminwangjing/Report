...
$(document).ready(function () {
                    ...
                    "change": function (e) {
                        $.ajax({
                            ...
                            success: function (res) {
                                ...
                                //let totalpay = 0;
                                //    __multipaydetail.forEach(i => {
                                //        console.log("amount", i.AmmountSys)
                                //        if (i.AmmountSys > 0) {
                                //            console.log("aomunt", i.AmmountSys)
                                //            totalpay += num.toNumberSpecial(i.AmmountSys);
    ...
    function dataFromCustomerIncomingPayment(vendorId) {
        ...
        $.get("/Incomingpayment/GetSaleAR", { customerID: vendorId, type }, function (resp) {
            ...
            console.log("Test= i",txtPayment,countClick);
    ...
    function saveInComing() {
        ...
        master.BasedOnID=$("#baseOnId").val();
        master.CopyType= $("#copytype").val();
        ...
        $.ajax({
            url: "/Incomingpayment/SaveIncomingPayment",
            type: "Post",
            dataType: "Json",
    ...
    function checkbox(e, _this) {
        ...
        if ($(_this).prop("checked") == true) 
        {
            ...
            $("#totalamount_due_asing").val(curFormat(num.toNumberSpecial(total(totalSummary, "TotalPayment"))));
            

            totalPayment.html(`<input class='form-control' value='${num.toNumberSpecial(e.data.TotalPayment)}'/>`);
            ...
            if (item[prop].toString().includes(","))
             {
                ...
                const amountSys = num.toNumberSpecial(item[prop].toString().split(".")[0].split(',').join('')) * num.toNumberSpecial(item.ExchangeRate);
                total += amountSys;
            }
            // console.log("item[prop]",(item[prop]))
            const amountSys = num.toNumberSpecial(item[prop]) * num.toNumberSpecial(item.ExchangeRate);
            total += amountSys;
        });
        return total;
    }
    ...
    function findIncomingPaymentToCancel(invoiceNumber, success) {
        ...
        if (invoiceNumber !== null) {
            $.get("/incomingpayment/getbyinvoice", { invoiceNumber, seriesID }, success);
        }
    }
    ...
    function getIncomingPaymentbyInvoice(_res) {
        ...
        let sum = 0;
        $.each(_res.IncomingPaymentDetails, function (index, item) {
            item.Date = item.Date.split("T")[0]
            sum += item.Totalpayment;
        });
        $list_incoming_payment_cancel.bindRows(_res.IncomingPaymentDetails);
    ...
    function cancelIncomingInvoice(invoice, success) {
        const seriesDID = _seriesDID;
        $.post("/incomingpaymenthistory/cancel", { invoice, seriesJE: JSON.stringify(__singleElementJE), seriesDID }, success);
    }
    // copy from
    $("#copy-from").change(function(){
        if(parseInt(this.value)!=0)
        {
            DialogIncomingpaymentOrder();
            this.value=0;
        }
    });

    function DialogIncomingpaymentOrder()
    {
        let dialog = new DialogBox({
            button: {
                ok: {
            ...
            content: {
                selector: "#incomingPaymentOrder-content"
            }
        });

        dialog.invoke(function () {
            let $incomingpayorder = ViewTable({
                keyField: "ID",
                selector: dialog.content.find("#list-incomingPaymentOrder"),
                indexed: true,
                paging: {
                    pageSize: 10,
                    enabled: true
                },
                visibleFields: ["InvoiceNumber", "DocumentType","Customer","PostingDate","TotalAmountDue","ApplyAmount"],
                actions: [
                    {
                        template: "<i class='fas fa-arrow-circle-down'></i>",
                        on: {
                            "click": function (e) {
                                ...
                                $.get("/Incomingpayment/CopyIncomingPayOrder",{id:e.data.ID,},function(res){
                                    __multipaydetail=[];
                                  
                                    let multotalpay=0;
                                    res.incompay.MultPayIncommings.forEach(i => {
                                        __multipaydetail.push(i);
                                        $multipayment.disableColumns(i.LineID, ["Amount"]);
                                        $multipayment.disableColumns(i.LineID, ["Currency"]);
                                        multotalpay+=i.AmmountSys;
                                    });
                                    $multipayment.clearRows();
                                    $multipayment.bindRows(__multipaydetail);
                                  
                                    
                                    $("#type").val(res.incompay.Type);
                                    $("#show-list-vendor").val(res.incompay.CustomerName).prop("disabled", true);;
                                    $("#vendorID").val(res.incompay.CustomerID);
                                    $("#txtref_no").val(res.incompay.Ref_No).prop("disabled", true);
                                    $("#txtUser").val(res.incompay.UserName).prop("disabled", true);
                                    // $("#txtInvoice").val(res.incompay.SeriesID);
                                    // $("#next_number").val(res.incompay.Number);
                                   // $("#paymentMeans").val(`${res.PaymentMeans.Type} ${_res.GLAccount.Code} - ${_res.GLAccount.Name}`).prop("disabled", true);
                                    $("#txtPostingdate").val(res.incompay.PostingDate.split("T")[0]).prop("disabled", true);
                                    $("#txtDocumentDate").val(res.incompay.DocumentDate.split("T")[0]).prop("disabled", true);
                                    $("#currency_text").text(res.IncompayOrderDetail[0].SysName)
                                    $("#total_amount").val(multotalpay);
                                    $("#selectall").prop('checked',true).prop("disabled", true);
                                    $("#txtremark").val(res.incompay.Remark);
                                    $("#baseOnId").val(res.incompay.IncomingPaymentID);
                                    $("#copytype").val(res.ConpyType);
                                    $("#branch").val(res.BranchID);
                                    details=[];
                                    
                                   
                                   // console.log("Data 2=>",countClick)
                                    txtPayment=[];
                                   
                                    let amountSys=0;
                                    res.IncompayOrderDetail.forEach(item=> {
                                        amountSys+=item.TotalPayment;
                                        details.push(item);
                                        txtPayment.push({
                                            ID: item.IncomingPaymentCustomerID,
                                                txtTotalPayment: item.TotalPayments,
                                                cashDic:item.CashDiscounts,
                                                totalDiscounts:item.TotalDiscounts
                                        });
                                    });
                                   
                                    $("#totalamount_due_asing").val(amountSys);
                                    $list_incoming_payment_details.clearRows();
                                    $list_incoming_payment_details.bindRows(details);
                                    details.forEach(i=> { 
                                        $list_incoming_payment_details.disableColumns(i.IncomingPaymentCustomerID, ["CheckPay"]);
            ...
            loadScreen()
            $.get("/Incomingpayment/GetIncomingPayOrder",{cusid: parseInt($("#vendorID").val())}, function (resp) {
                $incomingpayorder.clearRows();
                $incomingpayorder.bindRows(resp);
                $("#incomingpayorderSearch").keyup(function ()
                {
                    let __value = this.value.toLowerCase().replace(/\s/g, "");
                    let rex = new RegExp(__value, "i");
                    let __customers = $.grep(resp, function (person) {
                       
                        return person.InvoiceNumber.match(rex) 
                              || person.DocumentType.toLowerCase().replace(/\s/g, "").match(rex)
                              || person.Customer.toLowerCase().replace(/\s/g, "").match(rex)
                              || person.PostingDate.toLowerCase().replace(/\s/g, "").match(rex)
                             
                    });

                    $incomingpayorder.bindRows(__customers);
                });
                loadScreen(false)
            });
        });
