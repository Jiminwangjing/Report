...
$(document).ready(function () {
    ...
    $(".btnFilter").click(function () {
        onGetOutgoingPayment(function (outgoingPaymentOrder) {
            viewTable(outgoingPaymentOrder);
        });
    })
    ...
    function viewTable(data) {
        $("#list_outgoingPaymentOrder").dxDataGrid({
            //dataSource: data,
            //paging: {
            //    pageSize: 10
            //},
...
function printPdf() {
    ...
    window.open("/DevPrint/PreviewOutgoingPaymentOrderReport?status=" + status + "&fromDate=" + datefrom + "&toDate=" + dateto + "", "_blank");
}
function onGetOutgoingPayment(success) {
    const dateFrom = $("#txtdatefrom").val();
    const dateTo = $("#txtdateto").val();
    const status = $("#status").val();
    $.get("/Outgoingpayment/OutgoingpaymentOrderHistoryFilter", { status, DateFrom: dateFrom, DateTo: dateTo }, success);
}
