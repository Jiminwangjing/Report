...
function printPdf() {
    ...
    window.open("/DevPrint/PreviewIncommingPaymentReport?status=" + status + "&fromDate=" + datefrom + "&toDate=" + dateto + "", "_blank");
}
function printPdf3(LineID) {
    window.open("/DevPrint/PreviewIncommingPayment?LineID=" + LineID + "", "_blank");
}
function onGetIncomingPayment(success) {
    const dateFrom = $("#txtdatefrom").val();
    const dateTo = $("#txtdateto").val();
    const status = $("#status").val();
    $.get("/Incomingpayment/IncomingpaymentHistoryFilter", { status, DateFrom: dateFrom, DateTo: dateTo }, success);
}
