...
$(document).ready(function () {
    ...
    $(".show-list-vendor").click(function () {
        ...
        $.ajax({
            url: "/PurchaseOrder/GetBusinessPartners",
            type: "Get",
            dataType: "Json",
                ...
                actions: [
                    {
                        ...
                        on: {
                            "click": function (e) {
                                copyid = e.data.ID;
                                $(".show-list-vendor").prop("disabled", false);
    ...
    //get currency
    $.ajax({
        url: "/PurchaseOrder/GetCurrencyDefualt",
        type: "GET",
        dataType: "JSON",
    ...
    $("#txtcurrency").change(function () {
        $.ajax({
            url: "/PurchaseOrder/GetFilterLocaCurrency",
            type: "Get",
            dataType: "Json",
    ...
    $("#submit-data").click(function () {
        itemMaster.CopyKey = $("#txtcpoykey").val();
        itemMaster.BaseOnID = parseInt($("#baseonid").val());
        freightMaster.FreightPurchaseDetials = freights.length === 0 ? new Array() : freights;
        itemMaster.WarehouseID = isNaN(parseInt($("#txtwarehouse").val())) ? 0 : parseInt($("#txtwarehouse").val());
        ...
        itemMaster.PurchaseOrderDetails = orderDetail.length === 0 ? new Array() : orderDetail;
        ...
        dialogSubmit.confirm(function () {
            ...
            $.ajax({
                ...
                type: "POST",
                dataType: "JSON",
                data: { purchase: JSON.stringify(itemMaster), Type: type, copytype: copytype },
                success: function (response) {
    ...
    $("#next_number").on("keypress", function (e) {
        if (e.which === 13 && !!$("#txtInvoice").val().trim()) {
            $.ajax({
                url: "/PurchaseOrder/FindPurchaseOrder",
    ...
    $(window).on("keypress", function (e) {
        if (e.which === 13) {
            if (document.activeElement === this.document.getElementById("txtbarcode")) {
                ...
                $.get("/PurchaseOrder/GetItemDetails", { itemId: 0, curId: itemMaster.PurCurrencyID, barcode: activeElem.value.trim() }, function (res) {
                    if (res.IsError) {
                        new DialogBox({
                            content: res.Error,
                        });
    ...
    function getPurchaseItemMaster(_master) {
        if (_master.Error) {
            ...
            $.get("/PurchaseOrder/Getbp", { id: _master.PurchaseOrder.VendorID }, function (cus) {
                if (copytype != "PR")
            ...
            $.ajax({
                url: "/PurchaseOrder/GetItemMasterData",
                type: "GET",
                dataType: "JSON",
    ...
    function getFreight(success) {
        $.get("/PurchaseOrder/GetFreights", success);
    }
    ...
    function chooseItemDetail(e) {
        getItemMasterDetials(e.data.ID, function (res) {
            ...
            // if (isValidArray(orderDetail)) {
            //     var isExisted = findArray("ItemID", res.ItemID, "UomID", res.UomID, orderDetail);
            //     if (isExisted) {
            //         var qty = isExisted.Qty + 1;
            //         updateData(orderDetail, "ItemID", res.ItemID, "UomID", res.UomID, "Qty", qty);
            //         updateData(orderDetail, "ItemID", res.ItemID, "UomID", res.UomID, "OpenQty", qty);
            //         $listItemChoosed.updateColumn(isExisted.LineIDUN, "Qty", qty);
            //         e.data = isExisted;
    ...
    function getItemMasterDetials(itemid, success) {
        $.get("/PurchaseOrder/GetItemDetails", { itemid, curId: itemMaster.PurCurrencyID, barcode: "" }, success);
    }
    ...
    function setSummary(data) {
        ...
        if (isValidArray(orderDetail)) {
            orderDetail.forEach(i => {
                ...
                updateData(orderDetail, "LineIDUN", i.LineIDUN, "UomID", i.UomID, "FinDisValue", itemMaster.DiscountValue);
    ...
    $("#copy-from").change(function () {
        ...
        $.get("/PurchaseOrder/GetPurchaseQuotationCopy", { id: copyid, type: value }, function (res) {
            switch (value) {
                case 1:
                    copyData(res, "#container-list-item-copy-pr", ".item-copy-pr", "Purchase Requests");
                    break
                case 2:
                    copyData(res, "#container-list-item-copy", ".item-copy", "Purchase Quotation");
                    break
            }
        })
    });
    function copyData(data, dialogContainer, tableContainer, caption) {
        ...
        dialog.invoke(function () {
            const itemMasterCopy = ViewTable({
                ...
                visibleFields: ["DocType", "Invoice", "ValidUntil", "DocumentDate", "ReqiuredDate", "RequesterUsername", "RequesterCode", "RequesterName", "PostingDate", "CurrencyName", "SubTotal", "BalanceDue", "Remark"],
                actions: [
                    {
                        ...
                        on: {
                            "click": function (e) {
                                ...
                                if (type == 1) {
                                    $.get("/PurchaseRequest/CopyPurchaseRequest", { seriesID: e.data.SeriesID, number: e.data.Invoice }, function (res) {
                                        copytype = "PR";
                                        $("#baseonid").val(res.PurchaseOrder.BaseOnID);
                                        $("#txtcpoykey").val("PR-" + res.PurchaseOrder.Number);
                                        res.PurchaseOrder.VendorID = itemMaster.VendorID;// noted for Purchase Request only
                                        getPurchaseItemMaster(res);

                                    })
                                }
                                if (type == 2) {
                                    $.get("/PurchaseQuotation/CopyPurchaseQuotation", { seriesID: e.data.SeriesID, number: e.data.Invoice }, function (res) {
                                        copytype = "PQ";
                                        $("#baseonid").val(res.PurchaseOrder.BaseOnID);
                                        $("#txtcpoykey").val("PQ-" + res.PurchaseOrder.Number);
                                        getPurchaseItemMaster(res);
