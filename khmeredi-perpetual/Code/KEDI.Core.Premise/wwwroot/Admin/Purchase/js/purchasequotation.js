...
$(document).ready(function () {
    ...
    $("#txtcurrency").change(function () {
        $.ajax({
            url: "/PurchaseQuotation/GetFilterLocaCurrency",
            type: "Get",
            dataType: "Json",
            data: { CurrencyID: this.value },
            success: function (res) {
                $.get("/PurchaseQuotation/GetDisplayFormatCurrency", { curId: res[0].CurrencyID }, function (resp) {
    ...
    $("#submit-data").click(function () {
        ...
        /*itemMaster.CopyType = $("#CopyType").val();*/
        ...
        itemMaster.PurchaseQuotationDetails = orderDetail.length === 0 ? new Array() : orderDetail;
    ...
    $("#next_number").on("keypress", function (e) {
        if (e.which === 13 && !!$("#txtInvoice").val().trim()) {
            $.ajax({
                url: "/PurchaseQuotation/FindPurchaseQuotation",
    ...
    $(window).on("keypress", function (e) {
        if (e.which === 13) {
            if (document.activeElement === this.document.getElementById("txtbarcode")) {
                ...
                $.get("/PurchaseQuotation/GetItemDetails", { itemId: 0, curId: itemMaster.PurCurrencyID, barcode: activeElem.value.trim() }, function (res) {
                    if (res.IsError) {
                        new DialogBox({
                            content: res.Error,
                        });
    ...
    function getPurchaseItemMaster(_master) {
        if (_master.Error) {
            ...
            $.get("/PurchaseQuotation/Getbp", { id: _master.PurchaseOrder.VendorID }, function (cus) {
            ...
            $.ajax({
                url: "/PurchaseQuotation/GetItemMasterData",
                type: "GET",
                dataType: "JSON",
    ...
    function getFreight(success) {
        $.get("/PurchaseQuotation/GetFreights", success);
    }
    ...
    function currencyOption(id = null) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $.get("/PurchaseQuotation/GetDisplayFormatCurrency", { curId: $("#txtcurrency").val() }, function (resp) {
                    disSetting = resp.Display;
                })
            }
        });
    ...
    function getItemMasterDetials(itemid, success) {
        $.get("/PurchaseQuotation/GetItemDetails", { itemid, curId: itemMaster.PurCurrencyID, barcode: "" }, success);
    }
    ...
    $("#copy-from").change(function () {
        ...
        if (value == 1) {
            $.get("/PurchaseQuotation/GetPurchaseRequestCopy", function (res) {
                copyData(res);
            })
        }
    ...
    function copyData(data) {
        let dialog = new DialogBox({
            ...
            caption: "Purchase Request lists",
        });
        dialog.invoke(function () {
            const itemMasterCopy = ViewTable({
                ...
                visibleFields: ["DocType", "Invoice", "ValidUntil", "DocumentDate", "ReqiuredDate", "RequesterUsername", "RequesterCode", "RequesterName", "CurrencyName", "SubTotal", "BalanceDue", "Remark"],
                actions: [
                    {
                        ...
                        on: {
                            "click": function (e) {
                                ...
                                $.get("/PurchaseRequest/CopyPurchaseRequest", { seriesID: e.data.SeriesID, number: e.data.Invoice }, function (res) {
                                    ...
                                    copytype = "PR";
                                    res.PurchaseOrder.VendorID = itemMaster.VendorID;
                                    getPurchaseItemMaster(res);
                                })
                                dialog.shutdown();
                            }
