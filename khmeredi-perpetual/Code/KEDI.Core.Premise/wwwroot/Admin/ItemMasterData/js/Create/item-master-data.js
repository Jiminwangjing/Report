...
$(document).ready(function () {
    var _process = $("#txtProccess option:selected").val();
    if (_process !== "STANDARD") {
        $("#manMethod").val(1);//On every transaction
    }
    ...
    $("#btn-add").on("click", function () {
        ...
        coreMaster.submitData("/ItemMasterData/Create", ["itemMasterData", "itemAccountings", "properties"], function (res) {
    ...
    function createWarrantyTemplateDailog() {
        ...
        warDialog.confirm(function () {
            ...
            $.post("/ItemMasterData/CreateContractTemplate", { contract: dataWarranty }, function (res) {
                new ViewMessage({
                    summary: {
    ...
    function onGetWhAccTemplates(success) {
        $.get("/ItemMasterData/GetWarehouseAccounting", success);
    }
    ...
    $("#manItemBy").change(function () {
        ...
        if (this.value == 0) {
            ...
            //$("#manMethod").addClass("disabled");
            $("#manMethod").prop("disabled", true);
        } else {
            $(".seba").prop("hidden", false)
            //$("#manMethod").removeClass("disabled");
...
function getProperties(success) {
    $.get("/Property/GetActiveProperties", success);
}
...
function SelectChangProccess(val) {
    ...
    if (type !== "Standard") {
        ...
        $("#purcheckbox").prop("checked", true);
        $("#errmanItemBy").text("");
        $("#manItemBy").val(0).prop("disabled", false);
        coreMaster.updateMaster("Inventory", true);
        coreMaster.updateMaster("Purchase", true);
        coreMaster.updateMaster("Sale", true);
    ...
    // }
        ...
        $("#manItemBy").val(0).prop("disabled", true);
        $("#manMethod").val(0);
        $("#txtCost").val(0);
        $("#txtUnitPrice").val(0);
        $("#salecheckbox").prop("checked", true);
