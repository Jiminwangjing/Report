function BatchNoTemplate(config) {
    ...
    this.batchTemplate = function () {
        ...
        batchDialog.confirm(function () {
            $.post("/Sale/CheckBatchNo", { batches: JSON.stringify(__infos.batches) }, function (res) {
                if (res.IsApproved) {
                    $(".serailErrorMessage").text("");
                    $("#checkingBatchString").val("checked");
                            ...
                            if (__config.data.isCopy) {
                                if (__config.data.isCreditMemo) {
                                    fetchGetSerialDetialsCopy(e.data, true, function (res) {
                                        batchDialog.content.find("#ttSlt").val(res.TotalSelected);
                                        updateMaster(__infos.batches, "LineID", e.key, "BatchNoSelected", res);
                                        bindSelectedTableCopy(batchDialog, e.data);
                                    });
                                }
                                if (__config.data.isReturnDelivery) {
    ...
    function fetchGetSerialDetialsCopy(data, isCreditMemo, success) {
        $.get(
            "/Sale/GetBatchDetialsDeliveryCopy",
            {
                itemId: data.ItemID,
                uomId: data.UomID,
                saleId: data.SaleID,
    ...
    function bindSelectedTableCopy(batchDialog, data) {
