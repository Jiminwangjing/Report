$("#goto-main-menu").click(function (e) {
    ...
    main_menu.setting.button.ok.text = "Close";
...
function initFormSetting() {
    ...
    pos_setting.startup("before", async function (dialog) {
...
function initFormOpenShift() {
    ...
    open_shift.startup("before", function (dialog) {
        ...
        $.bindRows(content.find("table.open-cash-in"), currencies, "id", {
            html: [
                {
                    column: "input_cash",
                    insertion: "replace",
...
function initFormCloseShift() {
    ...
    close_shift.startup("before", function (dialog) {
        ...
        $.bindRows(content.find("table.close-cash-out"), currencies, "id", {
            html: [
                {
                    column: "input_cash",
                    insertion: "replace",
...
function initDataSetting(content) {
    ...
    if (setting.AutoQueue == false) {
        content.find('.receipt-auto-order').val(0);
    }
    else {
        content.find('.receipt-auto-order').val(1);
    }
}  
function processSetSetting(content) {
    setting.PrintReceiptOrder = content.find(".print-receipt-order").is(":checked");
    setting.PrintLabel = content.find(".print-receipt-label").is(":checked");
    ...
    setting.LocalCurrencyID = parseInt(content.find('.receipt-other-curr').val())
    ...
    if (content.find('.receipt-auto-order').val() == 0) {
        setting.AutoQueue = false;
    }
    else {
        setting.AutoQueue = true;
    }
    $.ajax({
...
function initDataReceipt(receipts) {
    ...
    $.each(receipts, function (i, receipt) {
        ...
        if (local_currency.symbol === 'KHR' || local_currency.symbol === '៛') {
            ...
            rec.amount = receipt.GrandTotal.toFixed(3);
        }
...
$('input[name="search-receipt-reprint"]').keyup(function () {
    ...
    let receipts = db.from("tb_reprint").where(w => {
        return w.receipt_no.toLowerCase().includes(query) || w.cashier.toLowerCase().includes(query) || w.table.toLowerCase().includes(query)
    });
