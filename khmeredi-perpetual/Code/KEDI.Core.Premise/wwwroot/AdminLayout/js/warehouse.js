...
class Warehouse extends Map
{
        ...
        if(!this.isValidArray(data)){
            if(!this.isValidObject(data)){
                ...
                if((item = this.get(from_table).get(key)) !== undefined){    
                    this.get(from_table).set(key, item);                  
                } 
            }        
        } else {
            let value = data[0];
            ...
            if(data.length === 2){    
                if(this.isValidObject(value)){
                    this.copy(from_table, to_table, value[key_name]);
                }
    ...
    cut(from_table, to_table, key){
        ...
        if(!this.isValidArray(key)){
            ...
            this.get(from_table).delete(key);
        }
    ...
    insert(table, value, key){
        if(key !== undefined){
            ...
            if(this.isValidObject(value)){
                this.get(table).set(value[key], value); 
            }
            ...
            if(this.isValidArray(value)){
                this.insert(table, value[0], value[1]);
            }
        }
    }

    //Modify existing object in table.
    update(table, value, key){    
        if(key !== undefined){
            if(this.isValidObject(value)){
            ...
            if(this.isValidArray(value)){
                this.update(table, value[0], value[1]);
            }
        }     
    }
