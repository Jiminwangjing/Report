...
$(document).ready(function () {
    var appMaster = JSON.parse($("#_listAppReport").text());
    ...
    function DataPagingAP() {
        ...
        paging.render(appMaster, function (filtered) {
            ...
            BindTableApp("#list-app-report", filtered.dataset);
        });
        paging.appendTo($("#paging-app-report"));
    }
    ...
    //Search Box
    $("#search-app-report").keyup(function () {
        let _input = this;
        let search = $(_input).val().toString().toLowerCase();
        $("#list-app-report tr:not(:first-child").filter(function () {
