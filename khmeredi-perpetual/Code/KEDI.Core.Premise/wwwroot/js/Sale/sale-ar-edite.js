...
$(document).ready(function () {
    ...
    const $listItemChoosed = ViewTable({
        ...
        columns: [
            {
                name: "UnitPrice",
                template: `<input class='font-size unitprice' type='text' />`,
                on: {
                    "keyup": function (e) {
                        $(this).asNumber();
            ...
            {
                name: "DisValue",
                template: `<input class='disvalue' type='text' />`,
                on: {
                    "keyup": function (e) {
                        $(this).asNumber();
                        calDisValue(e, this);
            ...
            {
                name: "DisRate",
                template: `<input class='disrate' type='text' />`,
                on: {
                    "keyup": function (e) {
                        $(this).asNumber();
                        calDisRate(e, this);
            ...
            {
                name: "Qty",
                template: `<input class='qty' type='text' />`,
                on: {
                    "keyup": function (e) {
                        $(this).asNumber();
            ...
            {
                name: "TaxGroupList",
                template: `<select class='taxgroup'></select>`,
                on: {
                    "change": function (e) {
            ...
            {
                name: "UoMs",
                template: `<select class='uom'></select>`,
                on: {
                    "change": function (e) {
                    ...
                    "click": function (e) {
                        if (e.data.SARDID > 0) {
                            ...
                            _dlg.confirm(function () {
                                //$.ajax({
                                //    url: "/Sale/DeleteSaleAREditDetail",
                                //    data: { id: e.data.SARDID },
                                //    method: "POST",
                                ...
                                for (let i = 0; i < ar_details.length; i++) {
                                    if (ar_details[i].LineID == e.key) {
                                        ar_details[i].Delete = true;
                                    }
                                }
                                setSummary(ar_details);
                                disInputUpdate(master[0]);
    ...
    $("#next_number").on("keypress", function (e) {
        if (e.which === 13 && !!$("#invoice-no").val().trim()) {
            $("#copy-from").prop("disabled", true);
            if ($("#next_number").val() == "*") {
                initModalDialog("/Sale/GetSaleAREditDisplay", "/Sale/FindSaleAREdite", "Fine A/R Edit", "SARID", "SaleAR", "SaleARDetails", 369);
            }
            else {
                $.ajax({
                    url: "/Sale/FindSaleAREdite",
    ...
    $("#save-draft").click(function () {
        ...
        item_master.ReceivedBy = parseInt($("#ware-id").val());
    ...
    $("#submit-item").on("click", function (e) {
        ...
        item_master.DocumentDate = $("#document-date").val();
        item_master.Remarks = $("#remark-id").val();
        item_master.SaleAREditeDetails = ar_details.length === 0 ? new Array() : ar_details;
        item_master.FreightSalesView = freightMaster;
        item_master.PriceListID = parseInt($("#cur-id").val());
        item_master.SeriesDID = parseInt($("#SeriesDetailID").val());
        item_master.SeriesID = parseInt($("#invoice-no").val());
        ...
        dialogSubmit.confirm(function () {
            ...
            $.ajax({
                url: "/Sale/CreateUpdateSaleAREdite",
                type: "post",
    ...
    $("#cancel-item").click(function () {
        ...
        const _this = this;
        $(_this).prop("disabled", true);
        $.get("/Sale/GetSaleAREditByInvoiceNo", { invoiceNo: item_master.InvoiceNumber, seriesID: item_master.SeriesID }, function (data) {
            $(_this).prop("disabled", false);
            if (data.AppliedAmount > 0) {
                ...
                dialogSubmit.confirm(function () {
                    cancelArEdit(item_master, saleArId)
    ...
    function cancelArEdit(item_master, saleArId) {
        $("#loading").prop("hidden", false);
        item_master.SeriesID = $("#invoice-no").val();
        item_master.SARID = 0;
        item_master.SaleAREditeDetails = ar_details;
        item_master.FreightSalesView.FreightSaleDetails = item_master.FreightSalesView.FreightSaleDetailViewModels;
        item_master.SaleAREditeDetails = item_master.SaleAREditeDetails.map(i => {
            return { ...i, SARDID: 0 }
        });
        item_master.SARID = 0;
        getARDownPayments(item_master.CusID, "used", saleArId, function (res) {
            ...
            $.post('/Sale/CancelSaleAREdit', { saleArEdited: JSON.stringify(item_master), saleArId }, function (res) {
                new ViewMessage({
                    summary: {
                        selector: "#error-summary"
                    }
    ...
    function bindItemCopy(_master, keyMaster, keyDetail, key, copyType = 0) {
        ...
        master[0] = _master[keyMaster];
        //master[0].BaseOnID = _master[keyMaster][key].BaseOnID;
    ...
    function getSaleItemMasters(_master) {
        ...
        if (!!_master) {
            ...
            if (_master.SaleAR.AppliedAmount > 0) {
                setDisabled();
                // setItemToDisabled([...itemArr, ".qty"]);
    ...
    function setDisabled() {
        /*        $("#barcode-reading").prop("disabled", true);*/
        ...
        $("#show-list-empre").addClass("csr-default").off();
        $("#show-list-empship").addClass("csr-default").off();
        $("#show-list-emprece").addClass("csr-default").off();
        $.each($("[data-date]"), function (i, t) {
