...
$(document).ready(function () {
            ...
            },
            {
                ...
                on: {
                    "keyup": function (e) {
                        ...
                        updatedetail(re_details, e.data.UomID, e.key, "OpenQty", this.value);
                        //calDisrate(e);
                        calDisvalue(e);
                        totalRow($listItemChoosed, e, this);
                        setSummary(re_details);
        ...
        actions: [
            {
                ...
                on: {
                    "click": function (e) {
                        ...
                        //$("#source-copy").val("");
                        if (e.data.ID > 0) {
                            let _dlg = new DialogBox({
                                content: "Are you sure you want to delete item [" + e.data.ItemNameKH + "] ?",
                                type: "yes-no"
                            });

                            _dlg.confirm(function () {

                                $listItemChoosed.removeRow(e.key);
                                for (let i = 0; i < re_details.length; i++) {
                                    if (re_details[i].LineID == e.key) {
                                        re_details[i].Delete = true;
                                    }
                                }
                                setSummary(re_details);
                                disInputUpdate(master[0]);
    ...
    $("#next_number").on("keypress", function (e) {
        if (e.which === 13 && !!$("#invoice-no").val().trim()) {
            ...
            $("#copy-from").prop("disabled", true);
            if ($("#next_number").val() == "*") {
                initModalDialog("/Sale/GetARReserveInvoiceEDTDisplay", "/Sale/FindARReserveInvoiceEDT", "Fine AR Reserve Invoice Editable", "ID", "ARReserveInvoice", "ARReserveInvoiceDetails", 369);
            }
            else {
                $.ajax({
                    url: "/Sale/FindARReserveInvoiceEDT",
                    data: { number: $("#next_number").val(), seriesID: parseInt($("#invoice-no").val()) },
                    success: function (result) {
                        if (result.ARReserveInvoiceDetails.length > 0) {
                            result.ARReserveInvoiceEditableDetails = result.ARReserveInvoiceDetails;
                            result.ARReserveInvoiceDetails.forEach(i => {
                                if (i.Qty != i.OpenQty) {
                                    setItemToDisabled([...itemArr, ".qty"]);
                                    // $("#submit-item").prop("disabled", false);
                                    $("#cancel-item").prop("disabled", true);
                            ...
                            new DialogBox({
                                caption: "Searching",
                                icon: "danger",
                                content: "Sale A/R REserve Editable Not found!",
                                close_button: "none"
                            });
    ...
    $("#item-id").click(function () {
        ...
        itemMasterDataDialog.invoke(function () {
            ...
            $.ajax({
                ...
                type: "Get",
                dataType: "Json",
                data: { PriLi: $("#cur-id").val(), wareId: $("#ware-id").val() },
                success: function (res) {
                    if (res.length > 0) {
    ...
    $("#btn-finddraft").click(function () {
        DraftDataDialog("/Sale/DisplayDraftReserveInvoiceEDT", "/Sale/FindDraftReserveInvoiceEDT", "Draft Detail", "LineID", "SaleDraft", "DraftARDetail");
    });
    //====show employee===
    ...
    function SubmitData(savetype) {
        ...
        item_master.ARReserveInvoiceEditableDetails = re_details.length === 0 ? new Array() : re_details;
        ...
        if (savetype == 1) {
            ...
            dialogSubmit.confirm(function () {
                ...
                $.ajax({
                    url: "/Sale/CreateARReserveInvoiceEditable",
                    type: "post",
            ...
            dialogsave.confirm(function () {
                ...
                item_master.DraftReserveInvoiceEditableDetails = re_details.length === 0 ? new Array() : re_details;
                $("#loading").prop("hidden", false);

                $.ajax({
                    url: "/Sale/SaveDraftReserveInvoiceEDT",
                    type: "POST",
                    dataType: "JSON",
                    data: {
    ...
    $("#cancel-item").click(function () {
        ...
        const _this = this;
        $(_this).prop("disabled", true);
        $.get("/Sale/GetSaleARReserveEDTByInvoiceNo", { invoiceNo: item_master.InvoiceNumber, seriesID: item_master.SeriesID }, function (data) {
            ...
            if (data.AppliedAmount > 0) {
                ...
                var dialogSubmit = new DialogBox({
                    content: "Are you sure you want to cancel this A/R Reserve Invoice Editable ?",
                    type: "ok-cancel",
                    icon: "warning"
                });
                dialogSubmit.confirm(function () {
                            ...
                            on: {
                                "click": function (e) {
                                    ...
                                    $.get(urlDetail, { draftname: e.data.DraftName, draftId: e.data.DraftID }, function (res) {
                                        ...
                                        res.ARReserveInvoice.DraffID = res.ARReserveInvoice.DraftID;
                                        ...
                                        res.ARReserveInvoiceEditableDetails = res.DraftARDetail;
                                        $("#draftname").val(res.ARReserveInvoice.Name);
                                        $("#submit-draft").html("Update-Draft");
                                        if (res.ARReserveInvoiceEditableDetails.length > 0) {
                                            res.ARReserveInvoiceEditableDetails.forEach(i => {
                                                i.DraffDID = i.DraftDetailID;
                                            })
    ...
    function cancelAr(item_master, saleArId) {
        ...
        // item_master.ID = 0;
        item_master.ARReserveInvoiceEditableDetails = re_details;
        item_master.FreightSalesView.FreightSaleDetails = item_master.FreightSalesView.FreightSaleDetailViewModels;
        getARDownPayments(item_master.CusID, "used", item_master.ID, function (res) {
            if (isValidArray(res)) {
            ...
            $.post('/Sale/CancelSaleARReserveEDT', { saleArEdited: JSON.stringify(item_master), saleArId }, function (res) {
                new ViewMessage({
                    summary: {
                        selector: "#error-summary"
                    }
                                    ...
                                    $.get(urlDetail, { number: e.data.InvoiceNo, seriesId: e.data.SeriesID }, function (res) {
                                        if (type == 369) {
                                            ...
                                            if (res.ARReserveInvoiceDetails.length > 0) {
                                                res.ARReserveInvoiceEditableDetails = res.ARReserveInvoiceDetails
    ...
    function getSaleItemMasters(_master, disabletable = true) {
        ...
        if (!!_master) {
            ...
            _master.ARReserveInvoiceEditableDetails.forEach(i => {
                i.DisValue = num.formatSpecial(i.DisValue, disSetting.Prices);
                i.Total = num.formatSpecial(i.Total, disSetting.Prices);
                i.TaxRate = num.formatSpecial(i.TaxRate, disSetting.Prices);
                i.TaxValue = num.formatSpecial(i.TaxValue, disSetting.Prices);
            ...
            re_details = _master.ARReserveInvoiceEditableDetails;
            $("#request_name").prop("disabled", true);
            $("#shipped_name").prop("disabled", true);
            $("#received_name").prop("disabled", true);
            ...
            if (isValidArray(_master.ARReserveInvoiceEditableDetails)) {
                $listItemChoosed.clearHeaderDynamic(_master.ARReserveInvoiceEditableDetails[0].AddictionProps);
                $listItemChoosed.createHeaderDynamic(_master.ARReserveInvoiceEditableDetails[0].AddictionProps);
                $listItemChoosed.bindRows(_master.ARReserveInvoiceEditableDetails);
            }
            if (master[0].Status == "close" || master[0].Status == "cancel") {
                setDisabled();
                setItemToDisabled([...itemArr, ".qty"]);
                $("#submit-item").prop("disabled", true);

                $("#submit-draft").prop("disabled", true);
            }
            // if (disabletable)
            //  setItemToDisabled([...itemArr, ".qty"]);
    ...
    function setSummary(data) {
        ...
        data.filter(d => !d.Delete).forEach(i => {
