...
const __filterType = {
    ...
    SaleType: 7 //{ Quotation = 1, Order = 2, Delivery = 3, AR = 4, CreditMemo = 5, ReturnDelivery = 6, ARDownPayment = 7 }
}
...
function bindSaleHistories(selector, data) {
    let $listhistory = ViewTable({
        ...
        actions: [
            {
                template: "<i class='fas fa-print fa-lg csr-pointer fn-green ml-4'></i>",
                on: {
                    "click": function (e) {
                        let ID = e.data.ID;
                        let method = "SaleArDownPaymentHistory";
                        let vatnumber = parseInt(e.data.TypeVatNumber);
                        if (vatnumber == 1) {
                            method = "SaleArDownPaymentHistoryNonVat01"
                        }
                        if (vatnumber == 2) {
                            method = "SaleArDownPaymentHi5storyVat01"
                        }
                        if (vatnumber >= 1) {
                            ...
                            method = "SaleArDownPaymentHistory";
