...
let quote_details = [],
    ...
    freights = [],
    freightMaster = {},
    ___SQ = JSON.parse($("#data-invoice").text()),
    disSetting = ___SQ.genSetting.Display;
let customerid = 0;
$(document).ready(function () {
    ...
    $('#invoice-no').change(function () {
        var id = ($(this).val());
        var seriesSQ = findArray("ID", id, ___SQ.seriesSQ);
        ___SQ.seriesSQ.Number = seriesSQ.NextNo;
        ___SQ.seriesSQ.ID = id;
        $("#DocumentID").val(seriesSQ.DocumentTypeID);
        $("#number").val(seriesSQ.NextNo);
        $("#next_number").val(seriesSQ.NextNo);
    });
    if (___SQ.seriesSQ.length == 0) {
        $('#invoice-no').append(`
    ...
    const $listItemChoosed = ViewTable({
        ...
        columns: [
            ...
            {
                name: "TaxGroupList",
                template: `<select class='form-control font-size'></select>`,
                on: {
                    "change": function (e) {
    ...
    $("#next_number").on("keypress", function (e) {
        if (e.which === 13 && !!$("#invoice-no").val().trim()) {
            if ($("#next_number").val() == "*") {
                findInvoice("/Sale/GetSaleQuotesDisplay", "/Sale/GetSaleQuoteDetailCopy", "Fine Sale Quotes", "SQID", "SaleQuote", "SaleQuoteDetails", 369);
            }
            else {
                $.ajax({
        ...
        $.get(urlMaster, { cusId: master[0].CusID }, function (res) {
            ...
            dialog.invoke(function () {
                const itemMaster = ViewTable({
                    ...
                    visibleFields: ["Code", "InvoiceNumber", "PostingDate", "Currency", "SubTotal", "TotalAmount", "Remarks"],
                    actions: [
                        {
                            template: `<i class="fa fa-arrow-alt-circle-down"></i>`,
                            on: {
                        ...
                        on: {
                            "click": function (e) {
                                $.get("/Sale/GetItemDetails", { PriLi: e.data.PricListID, itemId: e.data.ItemID, barCode: "", uomId: e.data.UomID }, function (res) {
                                    //if (isValidArray(quote_details)) {
                                    //    const isExisted = quote_details.some(i => i.ItemID === e.data.ItemID && i.UomID === e.data.UomID);
                                    //    if (!isExisted) {
                                    ...
                                    //$listItemChoosed.addRow(res[0])
                                    //}
                                    res[0].AddictionProps = e.data.AddictionProps;
    ...
    function getSaleItemMasters(_master) {
        if (!!_master) {
            ...
            quote_details = _master.SaleQuoteDetails;
            ...
            $("#next_number").val(_master.SaleQuote.InvoiceNumber);
            ...
            $("#sale-emid").val(_master.SaleQuote.SaleEmID);
            $("#sale-em").val(_master.SaleQuote.SaleEmName);
            ...
            $("#source-copy").val(_master.SaleQuote.BasedCopyKeys);
            ...
            setDate("#valid-date", _master.SaleQuote.ValidUntilDate.toString().split("T")[0]);
    ...
    $("#submit-item").on("click", function (e) {
        ...
        item_master.SaleQuoteDetails = quote_details.length === 0 ? new Array() : quote_details;
    ...
    $("#txt_copy").click(() => {
        ...
        dialogprojmana.invoke(function () {
                ...
                actions: [
                    {
                        ...
                        on: {
                            "click": function (e) {
                                ...
                                $.ajax({
                                    url: "/Sale/CopyProjectCostAnalysis",
                                    // data: { number: $("#next_number").val(), seriesID: ) },
                                    data: { number: e.data.InvoiceNumber, seriesID: e.data.SeriesID, comId: e.data.CompanyID },
                                    success: function (result) {
                                        result.SaleQuote.SQID = 0;
                                        result.SaleQuote.DisValue = result.SaleQuote.IvoiceDisvalue;
                                        result.SaleQuote.SubTotalSys = result.SaleQuote.SubTotalSys2;
                                        result.SaleQuote.ExchangeRate = result.SaleQuote.ExchangeRate2;
                                        result.SaleQuote.TotalAmountSys = result.SaleQuote.TotalAmountSys2;
                                        result.SaleQuote.FreightAmountSys = result.SaleQuote.FreightAmountSys2;
                                        result.SaleQuote.SubTotalBefDisSys = result.SaleQuote.SubTotalBefDisSys2;
                                        result.SaleQuote.SubTotalAfterDisSys = result.SaleQuote.SubTotalAfterDisSys2;
                                        // Detail
                                        //result.SaleQuoteDetails.forEach(i => {
                                        //    i.SubTotalSys = i.SubTotalSys2;

                                        //})
                                        $("#txt_copy").val(e.data.InvoiceNumber);
                                        getSaleItemMasters(result);
            ...
            $.get("/Sale/GetstoryProjectCost", { id: customerid }, function (res) {
    ...
    function selectSeries(selected) {
        $.each(___SQ.seriesSQ, function (i, item) {
    ...
    function totalRow(table, e, _this) {
        ...
        updatedetail(quote_details, e.data.UomID, e.key, "Total", totalWDis);
        updatedetail(quote_details, e.data.UomID, e.key, "TaxRate", e.data.TaxRate);
        updatedetail(quote_details, e.data.UomID, e.key, "TaxValue", vatValue);
        updatedetail(quote_details, e.data.UomID, e.key, "FinDisRate", e.data.FinDisRate);
        updatedetail(quote_details, e.data.UomID, e.key, "FinDisValue", fidis);
        updatedetail(quote_details, e.data.UomID, e.key, "TotalWTax", totalwtax);
        updatedetail(quote_details, e.data.UomID, e.key, "FinTotalValue", fitotal);
        updatedetail(quote_details, e.data.UomID, e.key, "TaxOfFinDisValue", taxoffinal);
    ...
    function calDisRate(e, _this) {
        ...
        updatedetail(quote_details, e.data.UomID, e.key, "DisRate", num.toNumberSpecial(_this.value));
