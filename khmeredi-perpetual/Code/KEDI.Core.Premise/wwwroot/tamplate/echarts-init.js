...
$(document).ready(function () {
    ...
    $(document).ready(function () {
        $.ajax({
            type: "GET",
            url: "/Chart/GetDashboardPurchase",
            data: "{}",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                $.each(response, function (i, item) {
                    indexpurchase.push(item.PurDate);
                    datapurchase.push(item.QtyPurchase);
                });
                var index = 0;
                for (var i = 1; i <= 12; i++) {
                    datapurchasemonth.push(0);
                    for (var j = 0; j < 12; j++) {
                        if (indexpurchase[j] == i) {                            
                            datapurchasemonth.splice(i-1, 1, datapurchase[index]);
                            index++;
                            break;
                        }
        ...
        $.ajax({
            type: "GET",
            url: "/Chart/GetDashboardSale",
            data: "{}",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                $.each(response, function (i, item) {
                    indexsale.push(item.InvDate);
                    datasale.push(item.QtySale);
                });
                var index = 0;
                for (var i = 1; i <= 12; i++) {
                    datasalemonth.push(0);
                    for (var j = 0; j < 12; j++) {
                        if (indexsale[j] == i) {
                            datasalemonth.splice(i-1, 1, datasale[index]);
                            index++;
                            break;
                        }
